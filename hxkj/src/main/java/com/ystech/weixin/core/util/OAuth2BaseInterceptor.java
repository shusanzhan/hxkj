...
public class OAuth2BaseInterceptor extends AbstractInterceptor {
	...
	public String intercept(ActionInvocation invocation) throws Exception {
		...
		WeixinGzuserinfo weixinGzuserinfo=null;
		if(Configure.prostatus==1){
			weixinGzuserinfo = weixinGzuserinfoManageImpl.findUniqueBy("openid", "oVdlDxNEHpbJ4Xh4R3M29C0eMhFw");
		}else{
			weixinGzuserinfo = (WeixinGzuserinfo)session.getAttribute("weixinGzuserinfo");
		}
		...
		if(null!=weixinGzuserinfo){
			...
			if(null!=weixinGzuserinfo2){
	            ...
	            if(null!=queryString){
	            	...
	            	session.setAttribute("codeBase", codeValue);
	            }
	            
	            //设置目标跳转连接，后续通过微信端获取到user后在进行页面跳转
	            session.setAttribute("resultBaseUrl", resultUrl);
	            //请求的路径  
	            String contextPath=request.getContextPath();  
	            response.sendRedirect(contextPath + "/wechatAuthBase/oAuth");
			}
		}
		return null;
	}
