...
public class SimpleHibernateDao<T> implements EntityDao<T>{
	 ...
	 * @param id
	 */
	public void deleteDuplicateDataByCustomerId(Serializable  customerId){
		if(null!=customerId){
			List<T> reList = findBy("customer.dbid", customerId);
	 ...
	 * @param id
	 */
	public void deleteDuplicateDataId(String dd, Serializable customerId){
		if(null!=customerId){
			List<T> reList = findBy(dd, customerId);
			if(null!=reList&&reList.size()>1){
				for (int i = 1; i < reList.size(); i++) {
					T t = reList.get(i);
					delete(t);
	 ...
	 * @return
	 */
	public Query executeSqlQuery(String sql, Object... values) {
		Assert.hasText(sql);
