...
public class SwfUploadAction extends BaseController{
	...
	//文件上传！出现异常文能解决
	public void uploadFileCoupon() {
		File dataFile = null;
		String filePath=null;
		try{
			if (null!=file&&!file.getName().trim().equals("")) {// getName()返回文件名称，如果是空字符串，说明没有选择文件。
				dataFile = FileNameUtil.getResourceFileCoupon(fileFileName);
	...
	public void downLoad()  {
		try {
			...
			System.out.println("hello ++++fileName 1" + file.getName());
			...
			System.out.println("hello ++++fileName2" + file.getName());
			is.close();
			response.flushBuffer();
			outputStream.flush();
			outputStream.close();
			directlyOutput(request);
			System.out.println("hello ++++fileName3" + file.getName());
		} catch (Exception e) {
			e.printStackTrace();
		}
